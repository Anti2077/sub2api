...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	opsRoutingMonitorService := service.ProvideOpsRoutingMonitorService(userRepository, redisClient)
	opsService := service.ProvideOpsService(opsRepository, settingRepository, configConfig, accountRepository, userRepository, concurrencyService, gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, opsSystemLogSink, settingService, authCacheInvalidationWorker, apiKeyService, opsRoutingMonitorService)
	usageHandler := handler.NewUsageHandler(usageService, apiKeyService, opsService, settingService)
	redeemHandler := handler.NewRedeemHandler(redeemService)
	dailyLotteryRepository := repository.NewDailyLotteryRepository(client)
	dailyLotteryService := service.NewDailyLotteryService(dailyLotteryRepository, settingRepository, userRepository, client)
	dailyLotteryHandler := handler.NewDailyLotteryHandler(dailyLotteryService)
	incentiveService := service.NewIncentiveService(client, userRepository, groupRepository, dailyLotteryService, apiKeyAuthCacheInvalidator)
	...
	adminDailyLotteryHandler := admin.NewDailyLotteryHandler(dailyLotteryService)
