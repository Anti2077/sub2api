...
func (r *usageLogRepository) GetUserBreakdownStats(ctx context.Context, startTime, endTime time.Time, dim usagestats.UserBreakdownDimension, limit int) (results []usagestats.UserBreakdownItem, err error) {
	query := `
		SELECT
			...
			COALESCE(u.username_confirmed, FALSE) as username_confirmed,
			COALESCE(u.leaderboard_anonymous, FALSE) as leaderboard_anonymous,
			COUNT(*) as requests,
	...
	query += " GROUP BY ul.user_id, u.email, u.username, u.username_confirmed, u.leaderboard_anonymous ORDER BY " + orderBy + " DESC"
	if limit > 0 {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&row.Email,
			&row.Username,
			&row.UsernameConfirmed,
			&row.LeaderboardAnonymous,
			&row.Requests,
			&row.InputTokens,
			&row.OutputTokens,
			&row.CacheTokens,
